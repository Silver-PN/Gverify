package com.example.xverifydemoapp.visio
 ...
 * ```
 */
class QRCodeAndBarcodeAnalyzer(private var options: BarcodeScannerOptions?=null,private val callback: QrCodeListener) :  ImageAnalysis. Analyzer{
    ...
    private fun processBarcodes(barcodes: List<Barcode>) {
        if (barcodes.isNotEmpty()) {
            val qrValue  = barcodes[0].displayValue;
            if (!qrValue.isNullOrEmpty()) {
                val result = EidFacade.parserQrCode(qrValue)
    ...
    interface QrCodeListener{
        fun onSuccess(basicInformation: BasicInformation)
    }
}
